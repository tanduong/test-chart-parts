...
import {Axis, Chart, Rect, LinearScale, BandScale, Dimension, Text} from '@chart-parts/react';
import {AxisOrientation, VerticalTextAlignment, HorizontalAlignment} from '@chart-parts/interfaces';
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <BandScale name="x" bandWidth="band" domain="data.category" padding={0.05} range={Dimension.Width} />
        ...
        <Rect
          ...
          ariaDescription={({d}) => `Category ${d.category} value is ${d.amount}`}
          tabIndex={1}
          onMouseEnter={({index}) => {
          ...
          width={({band}) => band()}
          y2={({y}) => y(0)}
          fill={({index}) => (hoverRowIndex === index ? 'firebrick' : 'steelblue')}
        />
        {hoverRowIndex === undefined ? null : (
          <Text
            text={d => d.data[hoverRowIndex].amount}
            fill="black"
            x={({data, x, band}) => x(data[hoverRowIndex].category) + band() / 2}
